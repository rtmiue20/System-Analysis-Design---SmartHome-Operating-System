...
import {
    Bell, Home, Moon, PowerOff, Lightbulb, Snowflake, Speaker,
    ChevronRight, Zap, Clock, MapPin,
    LayoutDashboard, Layers, Settings, LogOut, Activity
} from 'lucide-react';

const Dashboard = () => {
    const navigate = useNavigate(); // Khởi tạo biến navigate để chuyển trang
    ...
    // TÌNH TRẠNG LỌC THIẾT BỊ VÀ ĐIỀU HƯỚNG SIDEBAR
    ...
    const handleToggleDevice = async (id, currentStatus) => {
        ...
        // GỌI API ĐỂ LƯU TRẠNG THÁI VÀO DATABASE
        try {
            ...
            fetchDashboardData(); // Rollback lại data nếu API gọi bị lỗi
        }
    };
    ...
    const menuItems = [
        ...
        { id: 'rooms', label: 'Phòng ban', icon: Layers },
        { id: 'automations', label: 'Tự động hóa', icon: Zap },
        { id: 'settings', label: 'Cài đặt', icon: Settings },
    ];

    // Lọc thiết bị theo Ngữ cảnh/Phòng (activeRoomFilter)
    ...
    // 6. Render UI
    return (
        <div className={`flex min-h-screen font-sans transition-colors duration-500 ${mainBgClass}`}>

            {/* THÊM SIDEBAR BÊN TRÁI */}
            <aside className={`w-20 md:w-64 flex-shrink-0 border-r flex flex-col transition-all duration-300 ${sidebarBg}`}>
                <div className="h-20 flex items-center justify-center md:justify-start md:px-8 border-b border-transparent">
                    <div className="w-10 h-10 bg-orange-500 rounded-xl flex items-center justify-center text-white shadow-md shadow-orange-500/30">
                        <Home size={24} />
                    </div>
                    <span className={`ml-3 text-xl font-extrabold hidden md:block ${headerTextClass}`}>Smarthome</span>
                </div>
                    ...
                    {menuItems.map((item) => {
                        ...
                        return (
                            ...
                            >
                                ...
                                <span className="font-semibold text-base hidden md:block">{item.label}</span>
                            </button>
                        );
                    })}
                </div>
                ...
                <div className="p-4 mb-4">
                    <button className="flex items-center gap-4 px-4 py-3.5 w-full rounded-2xl text-gray-500 hover:bg-red-50 hover:text-red-500 transition-all duration-200">
                        <LogOut size={22} />
                        <span className="font-semibold text-base hidden md:block">Đăng xuất</span>
                    </button>
                </div>
            </aside>
            ...
            <div className="flex-1 overflow-y-auto">
                <div className="max-w-7xl mx-auto px-6 md:px-10 lg:px-12 py-8">
                    ...
                    <header className="flex justify-between items-center mb-10">
                        <div>
                            {/* Chuyển Text tiêu đề qua Sidebar, để lại khoảng trống hoặc Breadcrumbs nếu thích */}
                                ...
                                scenes.map((scene, index) => {
                                    ...
                                    return (
                                        <button
                                            ...
                                            className={`flex items-center gap-3 px-6 py-3.5 rounded-2xl font-semibold transition-all duration-200 transform hover:scale-95 active:scale-90 ${index === 0
                    ...
                    <section className="mb-14">
                        ...
                        {/* Danh sách thiết bị (Đã được lọc) */}
                            ...
                            {rooms.length > 0 ? (
                                rooms.map((room, index) => {
                                    ...
                                    return (
                                        <div
                                            key={rId}
                                            onClick={() => navigate(`/rooms/${rId}`)} // GẮN CHUYỂN TRANG VÀO ĐÂY
                                            className="relative h-60 rounded-3xl overflow-hidden group cursor-pointer shadow-sm hover:shadow-lg transition-shadow"
                                        >
                                            ...
                                            <div className="absolute bottom-6 left-6 right-6 flex justify-between items-end">
                                                ...
                                                <div className="w-10 h-10 bg-white/20 backdrop-blur-md rounded-xl flex items-center justify-center text-white group-hover:bg-orange-500 transition-colors">
